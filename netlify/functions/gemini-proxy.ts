...
// Schemas and prompt text now live securely on the server-side.
...
const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    ...
    if (text) {
      ...
      response = await ai.models.generateContent({ model: "gemini-2.5-flash", contents: userPrompt, config });
    } else if (fileData) {
      const imagePart = { inlineData: { mimeType: fileData.mimeType, data: fileData.data } };
      response = await ai.models.generateContent({ model: "gemini-2.5-flash", contents: { parts: [imagePart, { text: extractionPromptText }] }, config });
    } else {
      return { statusCode: 400, body: JSON.stringify({ error: "Missing 'text' or 'fileData' in request body." }) };
    }
    
    const jsonText = response.text;
    if (!jsonText) {
      return { statusCode: 500, body: JSON.stringify({ error: "API returned an empty response text." }) };
    }
